...
class Client():
    ...
    def __init__(self, url: str = "", grpc_max_receive_message_length: int = 4194304):
        ...
        if (self.url == None or self.url == ""):
            ...
            if (self.url == ""):
                self.url = os.environ.get("grpcapiurl", "")
            self.jwt = os.environ.get("jwt", "")
            if (self.jwt == ""):
                uri = urlparse(self.url)
                if (uri.username == None or uri.username == "" or uri.password == None or uri.password == ""):
                    raise ValueError(
                        "No jwt environment variable and no credentials in url")
        if (self.url == None or self.url == ""):
            self.url = "grpc://grpc.app.openiap.io:443"
    ...
    async def Signin(self, username: str = None, password: str = None, ping: bool = True, validateonly: bool = False, longtoken: bool = False):
        ...
        if (password == None or password == ""):
            request.data.Pack(base_pb2.SigninRequest(jwt=username, ping=ping, agent="python",
                              version="0.0.36", validateonly=validateonly, longtoken=longtoken))
        else:
            request.data.Pack(base_pb2.SigninRequest(username=username, password=password, ping=ping,
                              agent="python", version="0.0.36", validateonly=validateonly, longtoken=longtoken))
    ...
    async def QueueMessage(self, queuename: str, payload, correlationId=None, striptoken=True, rpc=False):
        ...
        reply.data.Pack(queues_pb2.QueueMessageRequest(queuename=queuename, data=res,
            ...
            for filepath in files:
                ...
                if compressed == True:
                    with open(filepath, mode="rb") as content:
                        _files.append(
                            {"filename": filename, "compressed": compressed, "file": gzip.compress(content.read())})
                else:
                    with open(filepath, mode="rb") as content:
                        _files.append(
                            {"filename": filename, "compressed": compressed, "file": content.read()})
        q = workitems_pb2.PushWorkitemRequest(
            wiq=wiq, name=name, files=_files, wiqid=wiqid, nextrun=nextrun, priority=priority)
    ...
    async def PopWorkitem(self, wiq: str, includefiles: bool = False, compressed: bool = False):
        ...
        request.data.Pack(workitems_pb2.PopWorkitemRequest(
            wiq=wiq, includefiles=includefiles, compressed=compressed))
            ...
            for filepath in files:
                ...
                if compressed == True:
                    ...
                    with open(filepath, mode="rb") as content:
                        _files.append(
                            {"filename": filename, "compressed": compressed, "file": content.read()})
        uwi = workitems_pb2.UpdateWorkitemRequest(
            workitem=workitem, files=_files, ignoremaxretries=ignoremaxretries)
    ...
    async def Query(self, collectionname: str = "entities", query: dict = {}, projection: dict = {}, top: int = 100, skip: int = 0, orderby=None, queryas: str = None):
        ...
        if (result == None or result.results == None):
            return None
    ...
    async def ListCollections(self, includehist: bool = False):
        ...
        request.data.Pack(querys_pb2.ListCollectionsRequest(
    ...
    async def DropCollection(self, collectionname: str):
        ...
        request.data.Pack(querys_pb2.DropCollectionRequest(
    ...
    async def GetDocumentVersion(self, id: str, collectionname: str, version: int = 0, decrypt: bool = True):
        ...
        request.data.Pack(querys_pb2.GetDocumentVersionRequest(
            collectionname=collectionname, id=id, version=version, decrypt=decrypt))
        result: querys_pb2.GetDocumentVersionResponse = await protowrap.RPC(self, request)
        if (result == None or result.result == None or result.result == ""):
            return None
    ...
    async def UpdateDocument(self, query: dict, document: dict, collectionname: str = "entities"):
        ...
        if (result == None or result.opresult == None):
            return None
    ...
    async def Watch(self, collectionname, paths: list, callback):
        ...
        request.data.Pack(watch_pb2.WatchRequest(
            collectionname=collectionname, paths=paths))
